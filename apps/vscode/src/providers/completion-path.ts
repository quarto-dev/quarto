...
export class PathCompletionProvider implements vscode.CompletionItemProvider {
  ...
  public async provideCompletionItems(
    ...
    switch (context.kind) {
      case CompletionContextKind.ReferenceLink: {
        return Array.from(
          this.provideReferenceSuggestions(document, position, context)
        );
      }
      ...
      case CompletionContextKind.Link: {
        ...
        const isAnchorInCurrentDoc =
          context.anchorInfo && context.anchorInfo.beforeAnchor.length === 0;
        ...
        if (context.linkPrefix.length === 0 || isAnchorInCurrentDoc) {
          ...
          for await (const item of this.provideHeaderSuggestions(
            document,
            position,
            context,
            insertRange
  ...
  private getPathCompletionContext(
    ...
    if (linkPrefixMatch) {
      ...
      return {
        ...
        linkPrefix: prefix,
        linkTextStartPosition: position.translate({
  ...
  private *provideReferenceSuggestions(
    document: vscode.TextDocument,
    position: vscode.Position,
    context: CompletionContext
  ): Iterable<vscode.CompletionItem> {
  ...
  private async *provideHeaderSuggestions(
    document: vscode.TextDocument,
    position: vscode.Position,
    context: CompletionContext,
  ...
  private async *providePathSuggestions(
    ...
    const pathSegmentStart = position.translate({
      characterDelta: valueBeforeLastSlash.length - context.linkPrefix.length,
    });
    const insertRange = new vscode.Range(pathSegmentStart, position);

    const pathSegmentEnd = position.translate({
