  ...
  implements vscode.SelectionRangeProvider
{
  ...
  private async provideSelectionRange(
    document: vscode.TextDocument,
    ...
    const inlineRange = await this.getInlineSelectionRange(
      document,
      position,
      blockRange
    );
  ...
  private async getInlineSelectionRange(
    document: vscode.TextDocument,
    position: vscode.Position,
    blockRange?: vscode.SelectionRange
  ): Promise<vscode.SelectionRange | undefined> {
    return createInlineRange(document, position, blockRange);
  }

  private async getBlockSelectionRange(
    document: vscode.TextDocument,
    position: vscode.Position,
    headerRange?: vscode.SelectionRange
  ): Promise<vscode.SelectionRange | undefined> {
    const tokens = await this.engine.parse(document);

    const blockTokens = getBlockTokensForPosition(
      tokens,
      position,
      headerRange
    );
    ...
    let currentRange: vscode.SelectionRange | undefined = headerRange
      ? headerRange
      : createBlockRange(blockTokens.shift()!, document, position.line);
  ...
  private async getHeaderSelectionRange(
    document: vscode.TextDocument,
    position: vscode.Position
  ): Promise<vscode.SelectionRange | undefined> {
...
function getHeadersForPosition(
  ...
  const enclosingHeaders = toc.filter(
    (header) =>
      header.location.range.start.line <= position.line &&
      header.location.range.end.line >= position.line
  );
...
function createHeaderRange(
  ...
  parent?: vscode.SelectionRange,
  startOfChildRange?: vscode.Position
  ...
  if (onHeaderLine && isClosestHeaderToPosition && startOfChildRange) {
    ...
    return new vscode.SelectionRange(
      range.with(undefined, startOfChildRange),
      new vscode.SelectionRange(range, parent)
    );
    ...
    return new vscode.SelectionRange(
      contentRange.with(undefined, startOfChildRange),
...
function getBlockTokensForPosition(
  ...
  position: vscode.Position,
  parent?: vscode.SelectionRange
): MarkdownItTokenWithMap[] {
  const enclosingTokens = tokens.filter(
    (token): token is MarkdownItTokenWithMap =>
      !!token.map &&
...
function createBlockRange(
  ...
  if (block.type === "fence") {
    ...
    let startLine = document.lineAt(block.map[0]).isEmptyOrWhitespace
      ? block.map[0] + 1
      : block.map[0];
...
function createInlineRange(
  ...
  if (
    boldSelection &&
    italicSelection &&
    !boldSelection.range.isEqual(italicSelection.range)
  ) {
...
function createFencedRange(
  ...
  if (contentRange) {
    return new vscode.SelectionRange(
      contentRange,
      new vscode.SelectionRange(fenceRange, parent)
    );
  } else {
...
function createBoldRange(
  ...
  if (matches.length) {
    ...
    const contentAndStars = new vscode.SelectionRange(
      new vscode.Range(
        cursorLine,
        startIndex,
        cursorLine,
    ...
    const content = new vscode.SelectionRange(
      new vscode.Range(
        cursorLine,
        startIndex + 2,
        cursorLine,
...
function createOtherInlineRange(
  ...
  if (matches.length) {
    ...
    const cursorOnType =
      cursorChar === startIndex || cursorChar === startIndex + match.length;
    const contentAndType = new vscode.SelectionRange(
      new vscode.Range(
        cursorLine,
        startIndex,
        cursorLine,
...
function createLinkRange(
  ...
  if (matches.length) {
    ...
    const linkRange = new vscode.SelectionRange(
      new vscode.Range(
        cursorLine,
    ...
    const contentAndNearestType = new vscode.SelectionRange(
      new vscode.Range(
        cursorLine,
        indexOfType,
        cursorLine,
...
function getFirstChildHeader(
  ...
  if (header && toc) {
    let children = toc
      .filter(
        (t) =>
          header.location.range.contains(t.location.range) &&
          t.location.range.start.line > header.location.range.start.line
      )
